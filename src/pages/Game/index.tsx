...
import { SettingOutlined, StockOutlined, DollarOutlined, BarChartOutlined, UserOutlined } from "@ant-design/icons";
    ...
    <div className={styles["game"]}>
      <div className={styles["game-header"]}>
        ...
        <Space>
          <Select
            ...
            optionRender={(option) => (
              <Flex style={{ alignItems: "center", gap: 8 }}>
                <Image
                  src={getGenshinRoleImg(option.data.englishName)}
                  preview={false}
                  width={20}
                  height={20}
                  style={{ display: "flex" }}
          ...
          <Button icon={<UserOutlined />} onClick={() => bookRef?.current?.show()}>
            角色图鉴
          </Button>
      ...
      <Space className={styles["game-show"]} size={16}>
        {showData?.map((i, index) => (
          <div key={index} className={styles["wish-show-item"]}>
            <RoleCard
              {...(i as RoleCardProps)}
              picUrl={i.picUrl || getGenshinRoleImg(roleList.find((j) => i?.name === j?.name)?.englishName || "")}
            />
          </div>
        ))}
      </Space>
